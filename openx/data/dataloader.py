...
def make_dataloader(
    ...
    chunk_obs_with_action: bool = False,
    sailor_offset: int = 50,
):
    # Get all datasets
    train_datasets = dict()
    val_datasets = dict()
    ...
    def _stepify(ep, dataset_id):
        ...
        # Add sequences
        ep = transforms.chunk(
            ep, n_obs, n_action, chunk_obs_with_action=chunk_obs_with_action, obs_keys=obs_history_keys
        )
        if goal_conditioning == "sailor":
            ep = transforms.sailor_goal_relabeling(ep, sailor_offset)
