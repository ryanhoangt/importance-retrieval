...
def chunk(
    ep: Dict, n_obs: int, n_action: int, chunk_obs_with_action: bool = False, obs_keys: Optional[Sequence] = None
):
    """
    ...
    idx = tf.range(ep_len)
    if chunk_obs_with_action:
        obs_idx = tf.maximum(idx[:, None] + tf.range(-n_obs + 1, n_action), 0)
        obs_idx = tf.minimum(obs_idx, ep_len - 1)  # mask the actual indexes to not go over.
    else:
...
def sailor_goal_relabeling(ep: Dict, sailor_offset: int):
    ep_len = tf.shape(tf.nest.flatten(ep["observation"])[0])[0]
    rand = tf.random.uniform([ep_len])
    # set goal_idx to be within [-50, 50] of tf.range(ep_len)
    low, high = (
        tf.cast(tf.range(ep_len) - sailor_offset, tf.float32),
        tf.cast(tf.range(ep_len) + sailor_offset, tf.float32),
    )
    goal_idx = tf.cast(rand * (high - low) + low, tf.int32)
    goal_idx = tf.minimum(tf.maximum(goal_idx, 0), ep_len - 1)
    ep["goal"] = tf.nest.map_structure(lambda x: tf.gather(x, goal_idx), ep["observation"])
    ep["goal_action"] = tf.nest.map_structure(lambda x: tf.gather(x, goal_idx), ep["action"])
    ep["dist"] = tf.abs(goal_idx - tf.range(ep_len))
    # can be useful for debugging
    # ep["goal_idx"] = goal_idx
    # ep["idx"] = tf.range(ep_len)
...
def decode_and_augment(
    ...
    if "flow" in structure["observation"].keys() and "image" not in structure["observation"].keys():
        # flow only
        for key in structure["observation"]["flow"].keys():
            crop_size = structure["observation"]["flow"][key]
            image = tf.image.random_crop(step["observation"]["flow"][key], size=(1, *crop_size, 2))
            step["observation"]["flow"][key] = image
        return step
