...
class SailorEncoderRNN(nn.Module):
    """
    SailorEncoderRNN takes in a dictionary of inputs.
    """
    ...
    def __call__(self, batch, train: bool = True):
        # input: (o_t, a_t) trajectory
        x = Concatenate(flatten_time=False)(batch)  # (B, T, D)
        x = nn.RNN(nn.LSTMCell(features=400))(x)  # single layer for now-- SAILOR uses 2 layer RNN
        return self.mlp(x[:, -1])


class SailorDecoderRNN(nn.Module):
    """
    SailorDecoderRNN takes in a dictionary of inputs.
    """
...
class SailorPrior(nn.Module):
    """
    SailorPrior takes in a dictionary of inputs.
    """
    ...
    def __call__(self, batch, train: bool = True):
        # input: (o_t) trajectory
        x = Concatenate(flatten_time=False)(batch)  # (B, T, D)
        # concatenate 0th and T-1'th observation
        x = jnp.concatenate([x[:, 0], x[:, -1]], axis=-1)  # (B, 2*D)
        return self.mlp(x)
...
class ExpandTimeDim(nn.Module):
    """
    Given input of shape (B, D), expand to (B, T, D)
    """
