...
class ResNetDecoderV2(nn.Module):
    """ResNet Decoder Network
    Fixes padding.
    """
    ...
    def __call__(self, z, obs, goal: Optional[jax.Array] = None, train: bool = True):
        ...
        # These parameters work for multiples of 32 for the input image size.
        h, w, _ = obs.shape[-3:]
        scale_h, scale_w = h // 32, w // 32

        # A Stem Equivalent
        x = jnp.reshape(z, z.shape[:-1] + (1, 1, z.shape[-1]))
        x = jax.image.resize(x, shape=x.shape[:-3] + (scale_h, scale_w, x.shape[-1]), method="nearest")
        x = conv(512, (3, 3), padding="SAME")(x)
        x = norm(name="gn_init")(x)
        x = nn.relu(x)
        x = jax.image.resize(x, shape=x.shape[:-3] + (2 * scale_h, 2 * scale_w, 512), method="nearest")
        ...
        for i, block_size in reversed(list(enumerate(self.stage_sizes))):
            for j in reversed(list(range(block_size))):
                ...
                x = self.block_cls(
                    self.num_filters * 2**i,
                    strides=strides,
                    conv=partial(conv, padding="SAME"),
                    norm=norm,
                    act=act,
                    transpose=True,
                )(x)
...
class ResNet18DecoderV2(ResNetDecoderV2):
    stage_sizes: Sequence[int] = (2, 2, 2, 2)
    block_cls: ModuleDef = ResNetBlock
