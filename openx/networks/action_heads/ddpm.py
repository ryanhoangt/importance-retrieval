...
class DDPMActionHead(core.ActionHead):
    ...
    def loss(self, obs: jax.Array, action: jax.Array, mask: jax.Array, dataset_id: jax.Array, train: bool = True):
        ...
        loss_0 = jnp.mean(loss * mask * (1-jnp.expand_dims(dataset_id, axis=1))) / jnp.clip(jnp.mean(mask * (1-jnp.expand_dims(dataset_id, axis=1))), a_min=1e-5, a_max=None)
        loss_1 = jnp.mean(loss * mask * jnp.expand_dims(dataset_id, axis=1)) / jnp.clip(jnp.mean(mask * jnp.expand_dims(dataset_id, axis=1)), a_min=1e-5, a_max=None)
        info = dict(loss=loss, loss_0=loss_0, loss_1=loss_1)
        return loss, info
