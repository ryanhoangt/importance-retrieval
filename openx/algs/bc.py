...
class BehaviorCloningModel(nn.Module):
    ...
    def loss(self, batch, train: bool = True):
        obs = self.observation_encoder(batch, train=train)
        return self.action_head.loss(obs, batch["action"], batch["mask"], batch["dataset_id"], train=train)

    def loss_and_prediction(self, batch, train: bool = True, **kwargs):
        obs = self.observation_encoder(batch)
        loss, info = self.action_head.loss(obs, batch["action"], batch["mask"], batch["dataset_id"], train=train)
...
class BehaviorCloning(Algorithm):
    ...
    def train_step(
        ...
        grads, info = jax.grad(_loss, has_aux=True)(state.params) 
