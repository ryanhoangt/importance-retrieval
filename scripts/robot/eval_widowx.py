...
flags.Define_bool("use_goal", False, "Whether or not we are goal conditioned.")
flags.DEFINE_string("goal_path", None, "Path to a goal state")
flags.DEFINE_spaceseplist("goal_eep", [0.24, 0.15, 0.2], "Goal position")
flags.DEFINE_spaceseplist("initial_eep", [0.3, -0.1, 0.185, 0, -0.259, 0, -0.966], "Initial position")
...
def main(_):
    ...
    resize_shape = (256, 341)

    env_params = WidowXConfigs.DefaultEnvParams.copy()
    env_params.update(ENV_PARAMS)
    initial_eep = FLAGS.initial_eep
    env_params["start_state"] = list(initial_eep)
    widowx_client = WidowXClient(host="localhost", port=5556)
    widowx_client.init(env_params, image_size=256)  # todo: check

    env = WidowXGym(widowx_client, resize_shape=resize_shape)  # IMAGE_SIZE = (256, 341)
    env = wrap_env(
        env,
        structure=structure,
        dataset_statistics=dataset_statistics["square_10_good"],
        n_obs=n_obs,
        n_action=n_action,
    ...
    # Determine if we are goal conditioned
    if FLAGS.use_goal:
    ...
    if FLAGS.video_save_path is not None:
        ...
        clip = ImageSequenceClip(list(video), fps=1.0 / STEP_DURATION * 3)
