"""
A script for behavior retrieval
"""
...
flags.DEFINE_bool("use_action", False, "Whether to use the action in the retrieval.")
...
class ScoreArray(object):
    ...
    def __init__(self, init_max_ep: int = 1000, init_max_steps: int = 400):
    ...
    def update(self, ep_idx, step_idx, v):
        ...
        self.array[ep_idx, step_idx] = np.maximum(self.array[ep_idx, step_idx], v)
...
def main(_):
    ...
    # This is a bit complicated, but ensures we can maintain the device mesh properly
    batch_size = FLAGS.batch_size
    num_local_devices = jax.local_device_count()
    num_hosts = jax.device_count() // num_local_devices
    ...
    def pad_batch(batch):
        ...
        return batch, np.array([valid] * num_local_devices, dtype=np.int32)

    def unpad_batch_replicated(batch, valid):
        # NOTE: This assumes we have replicated sharding on the output!
        valid = jnp.reshape(valid, (num_hosts, num_local_devices))[:, 0]

        def _unpad(x):
            assert x.shape[0] == num_hosts * batch_size
            x_dims = x.shape[1:]
            x = jnp.reshape(x, (num_hosts, batch_size, *x_dims))
            return jnp.concatenate([x[i, : valid[i]] for i in range(num_hosts)])

        return jax.tree.map(_unpad, batch)
    ...
    expert_zs = []
    for i, (batch, valid) in tqdm.tqdm(enumerate(expert_ds), dynamic_ncols=True):
        rng = jax.random.fold_in(rng, i)
        z = jitted_embed_fn(batch, rng)
        z = unpad_batch_replicated(z, valid)
        expert_zs.append(z)
    ...
    play_dataloader_config["repeat"] = False
    ...
    def _score(batch, rng):
        ...
        diff = play_z[:, None, :] - expert_zs[None, :, :]  # (B_play, expert, D)
        score = -jnp.linalg.norm(diff, axis=-1)  # (B_play, expert)
        # We can take the max of the scores over all expert data per BR paper
        score = jnp.max(score, axis=-1)
        return score, {k: batch[k] for k in METADATA_KEYS if k in batch}

    jitted_score_fn = jax.jit(_score, in_shardings=(dp_sharding, None), out_shardings=(rep_sharding, rep_sharding))
    ...
    score_array = ScoreArray(init_max_ep=init_max_ep, init_max_steps=init_max_steps)
    metadata_dict = collections.defaultdict(dict)

    for i, (batch, valid) in tqdm.tqdm(enumerate(play_ds), dynamic_ncols=True):
        rng = jax.random.fold_in(rng, i)
        scores, metadata = unpad_batch_replicated(jitted_score_fn(batch, rng), valid)
    ...
    # Normalize in min/max ranges.
    max_const = np.max(score_array)
    min_const = np.min(score_array[~mask])
    score_array = (score_array - min_const) / (max_const - min_const)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"viz/{vae_name}_br_all_scores.png")
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(f"viz/{vae_name}_br_ep_scores.png")
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(f"viz/{vae_name}_br_step_scores.png")
    plt.clf()
    ...
    if "quality_score" in metadata_dict:
        ...
        plt.title("Behavior Retrieval")
        plt.tight_layout()
        plt.savefig(f"viz/{vae_name}_br_ep_quality_curve.png")
        plt.clf()
        ...
        plt.title("Behavior Retrieval")
        plt.tight_layout()
        plt.savefig(f"viz/{vae_name}_br_step_quality_curve.png")
        plt.clf()
        ...
        plt.savefig(f"viz/{vae_name}_br_retrieval_stats.png")
        plt.clf()
    ...
    if "task_id" in metadata_dict:  # Split retrieved data by task id and show differing percentages
        ...
        plt.savefig(f"viz/{vae_name}_br_retrieved_tasks_distribution.png")
        plt.clf()
        ...
        plt.ylabel("Num Retrieved")
        plt.legend()
        plt.savefig(f"viz/{vae_name}_br_task_by_step.png")
        plt.clf()
        ...
        plt.xlabel("Task ID")
        plt.ylabel("Num Retrieved")
        plt.savefig(f"viz/{vae_name}_br_retrieved_tasks_distribution_top_{num_tasks}.png")
        plt.clf()
        ...
        plt.ylabel("Num Retrieved")
        plt.legend()
        plt.savefig(f"viz/{vae_name}_br_task_by_step_top_{num_tasks}.png")
        plt.clf()
