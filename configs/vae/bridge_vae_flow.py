...
from openx.data.datasets.bridge import bridge_flow_dataset_transform
...
def get_config(config_str="16,128"):
    ...
    dataloader = dict(
        datasets=dict(
            bridge=dict(
                ...
                train_split="train",
                val_split="val",
                transform=ModuleSpec.create(bridge_flow_dataset_transform),
            ),
        ),
    ...
    alg = ModuleSpec.create(
        ...
        encoder=ModuleSpec.create(
            MultiEncoder,
            encoders={
                "observation->flow->agent": ModuleSpec.create(ResNet18, num_kp=64),
        ...
        decoder=ModuleSpec.create(
            ...
            decoders={"observation->flow->agent": ModuleSpec.create(ResNet18Decoder), 
                        "observation->state": None, 
                        "action": None},
        ),
        z_dim=z_dim,
